--***FINAL GROUP QUERY for APP TRADER**
...
SELECT 	
	...
	CASE
		WHEN CAST(p.price AS MONEY) > a.price::MONEY THEN p.price
		WHEN CAST(p.price AS MONEY) < a.price::MONEY THEN '19900'
		WHEN CAST (p.price AS MONEY) = a.price::MONEY THEN '10000'
		END AS final_purchase_price
FROM play_store_apps AS p
INNER JOIN app_store_apps AS a
ON p.name=a.name),
...
ORDER BY expected_profit DESC
LIMIT 10;
---Works Needs to be Cleaned Up


---SETH Tweeking for Final........
...
(SELECT
    ...
    END AS appstore_purchase_price,
    COALESCE(GREATEST(
        GREATEST(CAST(a.price * 10000 AS money), CAST('0' AS MONEY)),
        GREATEST(CAST(CAST(p.price AS money)*10000 AS money), CAST('0' AS MONEY)),
        '10000'::money
