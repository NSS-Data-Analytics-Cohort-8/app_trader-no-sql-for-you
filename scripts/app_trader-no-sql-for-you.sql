...
-- vvvvvvvv old code. kept as refrence
-- SELECT 
-- 	a.name AS apple_store_apps,
-- 	p.name AS play_store_apps, 
-- 	p.genres, a.content_rating,
...
-- 		END AS final_purchase_price,
-- 	COALESCE(GREATEST(CAST(CAST(p.price AS money)*10000 AS NUMERIC),a.price * 10000),'10000') :: MONEY
--         AS scnd_final_purchase_price
-- FROM play_store_apps AS p
-- INNER JOIN app_store_apps AS a
-- ON p.name=a.name),
-- c3 AS (
